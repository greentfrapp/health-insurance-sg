  ...
  <div class="flex p-10 max-h-screen justify-center w-screen">
    <div class="flex w-full gap-4 max-w-full" :style="{width: '1280px'}">
      <div class="shrink-0 flex flex-col w-[65ch] space-y-4">
        <div class="history-container shrink grow space-y-4 overflow-auto px-2 pb-16" ref="historyContainer">
          <div v-for="step in store.history.filter(s => !s.hidden)">
            ...
            <div v-else>
              <div class="px-4 py-2">
                <component v-if="step.formattedContent" :is="parseResponse(step.formattedContent as APIResponse)"
                class="max-w-prose space-y-1"></component>
              </div>
            </div>
          </div>
          <div v-if="store.streamBuffer"
...
watch(() => [store.streamBuffer, store.history.length], async () => {
